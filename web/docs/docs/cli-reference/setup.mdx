...
4. **Project scaffold** — project-level agent files are written into the current directory:
   ...
   The scaffolded architect skill includes the current planning contract, including the optional parallel architect-baseline guidance used when `[orchestration].parallel_planner_architect = true`. Run `kas setup --force` only when you intend to refresh those scaffold-managed skill and prompt files from the bundled templates.
