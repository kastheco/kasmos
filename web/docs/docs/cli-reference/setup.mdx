...
4. **Project scaffold** — project-level agent files are written into the current directory:
   ...
   The scaffolded architect skill includes the current planning contract, including the default-on parallel baseline guidance and opt-out handling for `[orchestration].parallel_planner_architect = false`. It also instructs architect agents to write the final `decision_audit` into `.kasmos/cache/<task>-architect.json` so hq can display the baseline-vs-planner decision record. Run `kas setup --force` only when you intend to refresh those scaffold-managed skill and prompt files from the bundled templates.
