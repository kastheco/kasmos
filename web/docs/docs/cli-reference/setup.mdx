...
For each selected harness, choose which agent roles it should fill: `coder`, `architect`, `reviewer`, `planner`. You can also configure model names and per-role tuning options specific to each program.
...
4. **Project scaffold** — project-level agent files are written into the current directory:
   ...
   The scaffolded planner and architect skills include the current planning contract: optional `[orchestration].planners` fan-out, draft-mode `planner_draft_finished` signals, planner draft cache consumption, and final `decision_audit.planner_drafts` metadata in `.kasmos/cache/<task>-architect.json` so hq can display the decision record. Architect scaffolds require `kas task validate-architect-meta <task>` to strictly validate that cache before signaling. Reviewer and master workflow policy also lives in their scaffolded skills; managed prompts add only task-specific context such as the project, review round, prior findings, and configured verification limits. Re-review prompts target prior findings and fixer-touched surfaces unless shared integration boundaries require a broader pass. When `architect` is configured for Claude or OpenCode, setup also creates the matching `architect.md` role prompt. Run `kas setup --force` only when you intend to refresh those scaffold-managed skill and prompt files from the bundled templates.
