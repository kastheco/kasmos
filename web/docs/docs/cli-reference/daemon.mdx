...
The canonical compact orchestration snapshot is available on `GET /v1/repos/{project}/live-status`. Its `active_agents` and `attention` arrays default to 20 items each. Pass a `cap` query parameter to override that per-list limit; non-positive values select the default, and values above the hard maximum of 100 are reduced to 100. The response reports omitted item counts in `truncated`. See the [live status contract](../guides/live-status-contract.mdx) for the complete schema and versioning policy.
