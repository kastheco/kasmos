...
Show the current daemon status: registered repos, active plans per repo, and the current repo's Linear trigger monitor summary when the command is run inside a kasmos repo.
...
When the current repo has `[linear.triggers]` enabled, `kas daemon status` also reads the local task store and prints a 24-hour Linear trigger summary:

```text
linear triggers: enabled (last poll 12s ago, 3 dispatched, 1 rejected, 0 errors)
```

Without repo-local Linear triggers config, the extra line is:

```text
linear triggers: disabled
```
...
The canonical compact orchestration snapshot is available on `GET /v1/repos/{project}/live-status`. Its agent rows carry the daemon's branch, worktree basename, wave task number, last activity, and paused state. Its `active_agents` and `attention` arrays default to 20 items each. Pass a `cap` query parameter to override that per-list limit; non-positive values select the default, and values above the hard maximum of 100 are reduced to 100. The response reports omitted item counts in `truncated`. Review feedback appears as attention only while the task is back in `implementing` for a fixer, not during re-review, readiness verification, or terminal states. A task blocked by `needs_decision` always contributes an attention item with the human decision reason, even though its lifecycle status is unchanged. See the [live status contract](../guides/live-status-contract.mdx) for the complete schema and versioning policy.

`kas monitor status --json [--project <name>]` prints the same live-status contract. When `--project` is omitted, the command selects the only registered repository; it requires `--project` when multiple repositories are registered. The default `kas monitor status` text output remains unchanged.
...
Reload the daemon configuration by sending `SIGHUP` to the daemon process. The daemon re-reads every registered repository's `.kasmos/config.toml`; runtime review-fix and readiness-gate settings take effect on the next daemon poll tick.
...
# deprecated compatibility setting; verify_failed is never promoted to approval
...
# daemon-level default for Linear trigger polling (seconds)
# repo-local [linear.triggers].poll_interval overrides this value
poll_interval_sec = 60.0
```
