...
Verification approvals bind both the reviewed task HEAD and the default-branch snapshot. See [`kas task recover`](./task#recover) for manual recovery and [`kas signal`](./signal) for raw payload requirements.
...
- `kas task pr <plan-file>` is idempotent and prints the created, adopted, or previously recorded PR URL.
