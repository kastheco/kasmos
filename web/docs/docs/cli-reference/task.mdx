...
When `[linear.receipts]` is enabled and the task has an explicit Linear link, transitions that match the configured event allowlist post a receipt comment to the linked Linear issue. The CLI waits for in-flight receipt deliveries to settle before returning. See [linear receipts](../guides/linear-receipts.mdx).
...
| `verify-approved` | `verify_approved` | only valid while task is in `verifying`; carries `origin: operator` and binds approval to live HEAD |
...
Unlike a master signal, `recover --action verify-approved` does not accept a caller-supplied reviewed SHA. It records operator origin and binds the approval to the branch's live HEAD at admission time.
