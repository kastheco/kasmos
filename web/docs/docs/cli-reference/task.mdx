...
On success the command prints the PR URL and persists it in the task store. The PR body is generated from the plan content (goal, architecture, tech stack) and git log (commits, changed files, diff stats) using `gh pr create`.
