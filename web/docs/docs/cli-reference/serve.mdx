...
The MCP endpoint accepts CORS requests only from the `null` origin used by the generated `kas monitor widget` file preview. Its preflight permits `POST` and `OPTIONS` plus the MCP session header. The preview performs the normal Streamable HTTP initialization handshake and reuses its MCP session ID; arbitrary web origins are not allowed.
...
The read-only `open_monitor` tool serves the Codex monitor widget and its structured snapshot. In multi-project mode its `projects` field follows the same live routing catalog as other MCP tools, so newly registered projects become selectable without restarting the server.
