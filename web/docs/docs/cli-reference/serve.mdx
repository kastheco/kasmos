...
The MCP instance action tools use the same daemon inventory first. `instance_pause`, `instance_resume`, `instance_restart`, and `instance_send` therefore work for daemon-owned SDK sessions as well as tmux sessions. `capture_pane` returns the daemon's structured SDK presentation turns when available, including every retained turn, and otherwise uses the daemon's terminal capture endpoint. Pause, resume, send, and capture fall back to the legacy persisted-instance implementation only when the title is not present in the live daemon inventory; restart requires a daemon-managed instance.
