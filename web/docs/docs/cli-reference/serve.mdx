...
The full MCP endpoint does not grant CORS access to standalone files or arbitrary web origins. Generated `kas monitor widget` files instead use `POST /v1/widget-preview/open-monitor` on the REST listener. That endpoint grants the `null` file origin only enough access to request the read-only monitor snapshot; it does not accept a tool name or route requests to any other MCP capability.
