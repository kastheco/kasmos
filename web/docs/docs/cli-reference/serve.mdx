...
The MCP endpoint accepts CORS requests only from the `null` origin used by the generated `kas monitor widget` file preview. That preview performs the normal Streamable HTTP initialization handshake and reuses its MCP session ID; arbitrary web origins are not allowed.
