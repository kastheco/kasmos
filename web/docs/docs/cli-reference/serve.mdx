...
| `POST` | `/v1/projects/{project}/instances/{title}/permission` | submit a permission decision for a pending prompt; body: `{ "decision": <0\|1\|2> }` where `0` = allow once, `1` = allow always, `2` = reject |
