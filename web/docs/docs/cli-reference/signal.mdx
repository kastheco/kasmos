...
> **Parallel architect baseline:** enabling `[orchestration].parallel_planner_architect` does not add a signal type. The advisory `architect baseline` runtime session writes only `.kasmos/cache/<plan-file>-architect-baseline.json`; the planner still emits `planner_finished`, and the final architect pass still completes with `elaborator_finished`.
