...
| `verify_approved` | `{"reviewed_sha":"<full-head-sha>"}` (master) or `{"origin":"operator"}` (override) | approve during verifying (→ `done`). Master admission is strict: `reviewed_sha` must equal live branch HEAD. Trusted overrides bind live HEAD and identify their origin. |
...
- **`verify_approved`**: master agents must provide a non-empty `reviewed_sha`. Manual recovery uses `{"origin":"operator"}`; internal bypasses use `force_promoted` or `auto`. Arbitrary origins are rejected.
