...
Audit events can be filtered by repeated `kind=` values, `task`, `instance`, `after`, `before`, and `limit`. `after` and `before` are RFC3339 timestamps.
...
Store the highest seen `id` for idempotent ingestion, and advance `after` to the newest processed `timestamp` on the next poll.
