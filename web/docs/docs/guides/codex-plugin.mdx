...
Replace `<path>` with the absolute or Codex-accessible path to the Kasmos repository root. The plugin connects to the local MCP host and does not require separate credentials.
