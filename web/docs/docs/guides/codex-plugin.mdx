...
In Codex, add the marketplace from a Kasmos checkout, install the plugin, and reload plugins:

```text
/plugin marketplace add <path>/integrations/codex
/plugin install kasmos@kasmos
/reload-plugins
```

Replace `<path>` with the absolute or Codex-accessible path to the Kasmos repository root. Codex discovers the catalogue at `integrations/codex/.agents/plugins/marketplace.json`; pass the containing `integrations/codex` directory, not the JSON file. The plugin connects to the local MCP host and does not require separate credentials.
