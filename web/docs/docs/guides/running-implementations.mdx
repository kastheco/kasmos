...
By default, planning remains planner-first: the planner writes the draft and emits `planner-finished`, then the architect pass starts. You can opt into a parallel baseline in `.kasmos/config.toml`:
...
With this enabled, plan start clears any stale `.kasmos/cache/<planSlug>-architect-baseline.json`, starts the planner, and starts a separate `architect-baseline` runtime session. The baseline session writes only that cache artifact and emits no lifecycle signals. After the planner writes the draft and emits `planner-finished`, the final architect pass reads the planner draft plus a valid cached baseline, or falls back to its normal inline self-baseline if the cache is missing or invalid. Coder waves then proceed normally.

The baseline cache is advisory, safe to delete, and not task-store state. Existing signal names do not change. If the planner draft is small enough for blueprint skip, kasmos may skip the final architect pass, leaving a produced baseline cache unused.
