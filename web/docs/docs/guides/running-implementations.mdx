...
Planning can run one planner or a configured planner list. With `[orchestration].planners` unset or empty, kasmos uses the legacy path: one planner writes the task store and emits `planner_finished`. With an explicit list, plan start clears stale `.kasmos/cache/<planSlug>-planner-<profile>.md` files, starts one draft-mode planner per profile, and lets the first listed planner update the task store as a preview.

To run multiple planners, set:
...
Each draft-mode planner emits gateway-only `planner_draft_finished` with `{"planner_id":"<profile>"}`. The processor aggregates those draft signals and internally advances planning when all expected profiles are done. The final architect pass reads the task-store plan plus planner draft caches, derives its own baseline inline, writes the final task-store plan, and records consumed drafts in `decision_audit.planner_drafts`. If the planner draft is small enough for blueprint skip, kasmos may skip the final architect pass, leaving produced draft caches unused.
...
In the TUI status bar, the wave label (`wave 1/2`) does not appear for blueprint-skip plans. The compact info header shows a single agent instance rather than per-wave task glyphs.
...
- **preview pane** — live output from the selected agent's terminal (tmux) or structured event stream (sdk)
- **compact info header** — wave progress, per-task status glyphs, and subtask completion counts; shown above the preview pane by default, toggle with `I`
...
- [waves](../concepts/waves.mdx) — how wave orchestration works internally
- [worktrees](../concepts/worktrees.mdx) — how each task gets an isolated git branch
- [agents and roles](../concepts/agents-and-roles.mdx) — architect, coder, and solo agent profiles
