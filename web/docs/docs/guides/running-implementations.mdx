...
By default, planning starts the planner and an advisory `architect-baseline` session together. Plan start clears any stale `.kasmos/cache/<planSlug>-architect-baseline.json`, starts the planner, and starts the baseline session concurrently. The baseline session writes only the cache artifact and emits no lifecycle signals. After the planner writes the draft and emits `planner-finished`, the final architect pass reads the planner draft plus a valid cached baseline, or falls back to its normal inline self-baseline if the cache is missing or invalid. Coder waves then proceed normally.

To restore planner-first behavior — where the planner finishes and emits `planner-finished` before the architect pass starts — set:
