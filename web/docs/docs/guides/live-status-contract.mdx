...
| `review_feedback` | The task store records outstanding review feedback for the task. |
...
The default cap is 20 items per `active_agents` and `attention` array. Callers may override it with `cap`; non-positive values select the default, and values above the hard maximum of 100 are reduced to 100.
