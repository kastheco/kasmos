...
A UI process may poll every 1–5 seconds when it reuses the latest snapshot between polls rather than treating this endpoint as an event stream; a model-driven poller must use a much slower cadence of at least 30 seconds. Arrays are capped and the payload contains summary state rather than logs, transcripts, or task bodies, preserving the compactness guarantee.
