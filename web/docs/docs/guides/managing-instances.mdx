...
**standalone sdk agents** are sessions started directly from the TUI with `N`, `s`, or `S` when the resolved execution mode is `sdk`. they run in-process inside the TUI and are written to the on-disk `state.json` as ordinary instance records — they are _not_ registered with the daemon.
