...
The project `.agents/skills/` scaffold is also where kasmos ships role-specific operating rules. For example, the architect skill documents parallel baseline mode so final architect passes know how to merge a valid `.kasmos/cache/<plan-file>-architect-baseline.json` artifact while preserving the normal task-store writer and signal flow. The same architect skill requires the final `.kasmos/cache/<plan-file>-architect.json` metadata to include `decision_audit` fields for hq display.
