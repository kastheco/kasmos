...
- **sdk** — agent is driven via its app-server JSON-RPC protocol; output is streamed to the preview pane and logs. available for claude and codex only; other programs fall back to tmux silently. best for automated wave work.
