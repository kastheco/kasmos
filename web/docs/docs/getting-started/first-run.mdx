...
- **preview pane** — live agent output with a compact info header above it showing runtime metadata for the selected instance
...
| `I` | toggle compact info header (on by default) |
...
- **sdk** — agent is driven via its app-server JSON-RPC protocol; output streams to the preview pane and audit log. available for claude and codex only; other programs fall back to tmux silently. best for automated wave work.
