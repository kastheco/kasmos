...
The default planning sequence is planner-first. With `[orchestration].parallel_planner_architect = false`, the planner finishes first, emits `planner-finished`, and the architect pass derives its own baseline inline while reading the planner draft.

When `parallel_planner_architect = true`, kasmos starts an advisory baseline beside the planner:
