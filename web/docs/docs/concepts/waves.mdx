...
2. when `[orchestration].planners = ["planner", "planner_x"]` is set, plan start clears stale `.kasmos/cache/<planSlug>-planner-<profile>.md` caches, spawns each listed profile in draft mode, and lets the first listed planner update the task store as a preview
