...
> **parallel architect baseline:** `[orchestration].parallel_planner_architect = true` does not add a status, execution phase, or event. During `plan_start`, kasmos may clear `.kasmos/cache/<plan-file>-architect-baseline.json` and start a separate `architect baseline` runtime session beside the planner. That cache is advisory input for the later architect pass; lifecycle transitions still depend on the existing `planner_finished` and `elaborator_finished` signals.
