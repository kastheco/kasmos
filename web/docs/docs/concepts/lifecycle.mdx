...
terminal approval is `verify_approved`, or `review_approved` when `auto_readiness_review = false`. After applying that transition, kasmos marks the task `done` and, when `auto_create_pr = true`, runs the shared PR creation service. The service uses the branch stored in task metadata, adopts an existing PR before attempting creation, and persists both the URL and outcome. PR creation failure does not roll back the lifecycle transition; recover with `kas task pr <plan-file>`. See [automatic PR creation](../daemon/pr-creation.mdx).
