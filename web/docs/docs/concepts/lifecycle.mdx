...
agent spawn options are resolved alongside these phase actions, not stored in signal payloads. for daemon-managed lifecycle spawns, `permission_default` inherits to `SkipPermissions=true` unless the role profile explicitly says `"prompt"` or `"bypass"`; local tui spawns inherit to prompt. the resolved value follows the instance through daemon status and live-preview records so pause/resume/restart paths do not guess.
...
> **parallel architect baseline:** the architect baseline is default-on and does not add a status, execution phase, or event. During `plan_start`, kasmos clears `.kasmos/cache/<plan-file>-architect-baseline.json` and starts a separate `architect baseline` runtime session beside the planner. That cache is advisory input for the later architect pass; lifecycle transitions still depend on the existing `planner_finished` and `elaborator_finished` signals. Set `[orchestration].parallel_planner_architect = false` to disable.
