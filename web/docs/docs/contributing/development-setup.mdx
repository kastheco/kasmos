...
This sets `git config core.hooksPath scripts/git-hooks`. If you already use a custom `core.hooksPath` (e.g. husky, lefthook), the installer refuses to overwrite — pass `--force` to override.
