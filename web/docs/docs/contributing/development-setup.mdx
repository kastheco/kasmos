...
- `Docs-Drift-Skip: <reason>` commit trailer on any commit in the push range — auditable, preserved in history.

CI runs the same check as a required status, so `--no-verify` lands you in a failing PR. Maintainers can override at branch protection level.
