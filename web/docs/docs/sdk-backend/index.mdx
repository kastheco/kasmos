...
`S` opens a multi-step spawn flow: first a harness picker (if multiple programs are configured), then a name/branch/path form. the execution mode is resolved from the **master profile** via:
