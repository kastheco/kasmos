...
**consequence:** if you set `execution_mode = "sdk"` for `opencode`, `gemini`, `amp`, or a custom program, kasmos will silently run it in tmux instead. the docs and `kas debug config` output will still show `"sdk"` from the profile (the normalized config value), but the runtime session is a tmux session. use `kas debug` to confirm the resolved mode before relying on sdk behavior.
...
prints the fully resolved `Config` struct as JSON, including the normalized `execution_mode` for each agent profile. use this to confirm that `"headless"` aliases were resolved to `"sdk"` and that unsupported programs received `"tmux"`.
