...
- **info pane (TUI):** with an sdk instance selected, the info pane shows current byte usage, line count, turn count, and cumulative eviction counters (`evicted_turns`, `evicted_lines`, `evicted_bytes`, `truncated_rows`).
- **web admin structured preview:** the preview panel header shows the evicted-turn count when non-zero as `(N evicted)`.
