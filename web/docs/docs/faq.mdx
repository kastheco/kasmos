...
| **sdk** | agent is driven via its app-server JSON-RPC protocol (claude or codex only) | partial — `SendKeys`/`TapEnter` deliver prompts via the transport; `Attach` and `SetDetachedSize` return `ErrInteractiveOnly` |
