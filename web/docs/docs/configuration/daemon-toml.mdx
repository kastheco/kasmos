...
| `auto_readiness_review` | bool | `true` | when `true`, reviewer approval transitions to `verifying` and the daemon spawns an independent master review. When `false`, approval flows directly to `done` and binds live HEAD with `verified_by = "auto"`. A later commit makes either approval stale and reopens verification. |
