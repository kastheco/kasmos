...
| `pr_base_branch` | string | — | branch that `gh pr create` targets. Empty omits `--base` entirely, so GitHub's default branch is used — set it when task branches merge into an integration branch instead. Read from the repo's own `.kasmos/config.toml`, so each repo the daemon serves gets its own value. |
