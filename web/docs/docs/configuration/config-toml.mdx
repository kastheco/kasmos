...
| `auto_readiness_review` | bool? | `true` | when `true`, reviewer approval transitions to `verifying` and spawns an independent master review. When `false`, approval flows directly to `done`, binding live HEAD with `verified_by = "auto"`. All `done` approvals remain HEAD-bound and become stale after a later commit. |
