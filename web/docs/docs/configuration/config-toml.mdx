...
| `parallel_planner_architect` | bool | `false` | when `true`, plan start launches a cache-only architect baseline session beside the planner so the later architect pass can compare the planner draft with an independent baseline |
...
`parallel_planner_architect` is opt-in. The default `false` preserves the current planner-first behavior: the planner writes the draft, emits `planner-finished`, and the architect pass starts afterward.

When enabled, the runtime sequence is:
