...
## human-decision block columns

Tasks can be blocked without changing their lifecycle status when an agent needs a human ruling. The task row stores `blocked_reason`, `blocked_source`, and `blocked_at`. A non-empty reason suppresses orchestration spawns until an operator applies a lifecycle transition, which clears all three fields. Review-fix cycle limits create this same block instead of repeatedly spawning fixers.
...
| `PUT` | `/v1/projects/{project}/tasks/{filename}/blocked` | set a decision block from JSON `{ "reason": "...", "source": "..." }` |
| `DELETE` | `/v1/projects/{project}/tasks/{filename}/blocked` | clear a decision block |
