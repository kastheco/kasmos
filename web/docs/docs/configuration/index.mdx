...
See [config.toml reference](./config-toml.mdx) for all fields.
...
See [daemon.toml reference](./daemon-toml.mdx) for all fields.
...
Runtime artifacts that are derived from agent work live under the repo-local `.kasmos/cache/` directory. Multi-planner runs write draft caches as `.kasmos/cache/<task>-planner-<profile>.md`. Architect passes write their wave metadata to `.kasmos/cache/<task>-architect.json`; newer metadata may include a schema-versioned `decision_audit` object with planner draft summaries, the architect's final decision, differences, and `planner_drafts` entries for each consumed draft. Cache files are not task-store state.
