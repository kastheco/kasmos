...
See [headless vs tmux](./comparison.mdx) for a full comparison.
...
- [how it works](./how-it-works.mdx) — internals: process lifecycle, env vars, the done channel
- [configuration](./configuration.mdx) — setting `execution_mode = "headless"` in `config.toml`
- [logs and output](./logs-and-output.mdx) — reading captured output from the buffer and log files
- [headless vs tmux](./comparison.mdx) — full interface comparison
