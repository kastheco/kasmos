...
The effective `max_review_fix_cycles` caps how many review→fix cycles the monitor will trigger per task. A repository's `[ui].max_review_fix_cycles` in `.kasmos/config.toml` takes priority; the top-level value in `daemon.toml` is the fallback. Set the effective value to `0` for unlimited cycles.
...
To override the fallback for one repository, set the project-local value:

```toml title=".kasmos/config.toml"
[ui]
...
Set project-local `[ui].max_review_fix_cycles = 0` to disable the cap for one repository, or set the daemon-level fallback to `0` for repositories without an override.
