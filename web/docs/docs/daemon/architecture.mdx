...
`InstanceStatus.worktree` is the absolute worktree path on the daemon's local control-plane wire. `InstanceStatus.paused` explicitly reports a paused instance and is distinct from `active`, which continues to mean running and not paused. before instance data reaches a model-visible live-status surface, the live-status contract narrows `worktree` to its basename so absolute local paths are never exposed there.
