...
flowchart TD
    ...
    Orchestrator -->|"spawn / control"| SDK
```
...
| `Theme` | `theme.Result` | resolved repo palette used by daemon-served sdk preview and capture output |
...
spawn actions are resolved at the daemon boundary before they reach the spawner. the daemon reads the repo's profile for the target role, resolves `permission_default` against the daemon default of bypassing permissions, and writes the final boolean into `loop.SpawnOpts.SkipPermissions`. the spawner and session transports only see that final value.
...
the same tracked instances are exposed over the control api as `api.InstanceStatus` rows. status rows include the resolved `skip_permissions` value when reported by a new daemon; clients treat a missing value from an older daemon as the legacy daemon default of bypassing permissions.

`InstanceStatus.worktree` is the absolute worktree path on the daemon's local control-plane wire. `InstanceStatus.paused` explicitly reports a paused instance and is distinct from `active`, which continues to mean running and not paused. The daemon live-status adapter forwards worktree, branch, task number, last activity, and paused state into the shared contract. Before instance data reaches a model-visible live-status surface, the contract narrows `worktree` to its basename so absolute local paths are never exposed there.
