...
this calls `gateway.Create(project, entry)` which inserts a row with `status = 'pending'`. in agent workflows, use mcp `signal_create` for the same gateway-backed result without shelling out to the cli. draft-mode planners should use `planner_draft_finished` with a payload such as `{"planner_id":"planner_x"}`; that signal is gateway-only and is not bridged through filesystem sentinels.
