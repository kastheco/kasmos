...
> **parallel architect baseline:** the optional `architect baseline` session is not a lifecycle phase and emits no signal. When `[orchestration].parallel_planner_architect = true`, `plan_start` clears the advisory cache and can spawn the planner plus the baseline session; normal progress still flows through `planner_finished` and then `elaborator_finished`.
