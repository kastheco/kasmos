...
|-------------|------------|---------|
| `needs_decision` | any agent | records a durable human-decision block without an FSM transition; suppresses future agent spawns and appears in live-status attention until an operator transitions the task |
...
| `verify_approved` | master agent / manual recovery | FSM transition `verifying → done`. Gateway payloads carry `reviewed_sha` and `reviewed_base_sha`; manual recovery resolves and submits that same proof. **No loop-cap signal is ever written to the gateway.** |
| `verify_failed` | master agent | FSM transition `verifying → implementing`, spawn fixer when automatic review fixes are enabled. A failed verdict is never promoted to approval. |
...
`needs_input`, `blocked`, and `needs_human` are accepted aliases for `needs_decision`. Its payload must provide a non-empty reason, normally `{"reason":"choose option a or b"}`; bare text is also accepted as the reason. A decision block is orthogonal to lifecycle status. The only unblock path is an operator lifecycle transition, which clears the block before the task proceeds. Reaching `max_review_fix_cycles` creates the same durable block with the last reviewer feedback, rather than starting another fixer.
...
Gateway approval is strict: a `verify_approved` payload must include `reviewed_sha` and `reviewed_base_sha`, and it is admitted only when both still equal the reviewed task HEAD and default-branch snapshot. A mismatch rejects approval and keeps verification active. Manual recovery resolves and submits the same bound payload; it does not receive privileged provenance merely because it came from the CLI or TUI. The internal `auto` admission is synthesized in-process.

Gateway payloads are agent-controlled, so they cannot establish operator or internal provenance. An `origin` value supplied through `signal_create` is not accepted as an override; gateway master approvals still require matching head and base proofs. Terminal and PR side effects stop if the verification record cannot be persisted.

Legacy or imported tasks without a stored branch use the canonical `plan/<task>` branch consistently for verification admission, drift detection, master startup, PR creation, and merge checks. Git/repository failures remain blocking errors; only a confirmed missing branch is treated as a deleted task branch.

Verification binds both the task commit and the default-branch commit. A change to either invalidates approval. Merge and PR creation pass the reviewed task object ID into the final git operation, so a branch ref that moves after the initial check is rejected instead of being merged or pushed.

PR admission uses the same recovery behavior as daemon drift detection: when it discovers stale verification on a `done` task, it clears the binding and applies `verification_stale` so the task returns to `verifying` instead of remaining terminal without a valid proof.

```json
{"reviewed_sha":"<full-head-sha>","reviewed_base_sha":"<default-branch-sha>"}
```
