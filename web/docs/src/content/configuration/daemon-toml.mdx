...
| `auto_readiness_review` | bool | `true` | the daemon intercepts reviewer approval and hands off to the master agent for a holistic readiness check before the task moves to `done`. Set to `false` to skip the readiness review phase. |
