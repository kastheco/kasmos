...
**Effective value resolution:** for each of these settings the daemon uses a two-tier priority:

1. **Project-local `config.toml`** (`<repo-root>/.kasmos/config.toml`) — when the field is explicitly set in that file it wins for the repo in question.
2. **User-level `daemon.toml`** (`~/.config/kasmos/daemon.toml`) — the fallback when no project-local override is present.

This means you can set conservative global defaults in `daemon.toml` (e.g. `readiness_max_verify_cycles = 3`) and tighten or loosen them per-project in `config.toml` (e.g. `readiness_max_verify_cycles = 1` for a fast-moving repo). Keep the values in sync across files when you want consistent behavior in both interactive TUI and headless daemon mode.
