...
> **`readiness_review` vs `master_review`:** `readiness_review` is the canonical phase key. The old key `master_review` is accepted as a backward-compatible alias and is transparently normalised to `readiness_review` at load time. New configs should use `readiness_review`.
