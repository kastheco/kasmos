...
| `auto_readiness_review` | bool? | `true` | intercept reviewer approval and hand off to the master agent for a holistic readiness check before the task transitions to `done`. Set to `false` to skip the readiness review phase. |
...
auto_readiness_review = true    # set to false to skip the master readiness gate
```
