...
fixer            = "fixer"
readiness_review = "master"   # opt-in; also accepted as master_review (backward-compat alias)
```
...
> **`readiness_review` vs `master_review`:** `readiness_review` is the canonical phase key. The old key `master_review` is accepted as a backward-compatible alias and is resolved to `readiness_review` at profile resolution time (not at load time). New configs should use `readiness_review`.
...
| `auto_readiness_review` | bool? | `false` | when `true`, intercept reviewer approval and hand off to the master agent for a holistic readiness check before the task transitions to `done`. Opt-in — default is disabled. |
...
auto_readiness_review = false   # set to true to enable the master readiness gate
```
