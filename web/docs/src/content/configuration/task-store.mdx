...
The task store is a **global, cross-project database**. Tasks from different repositories are separated by project name (derived from the repo root directory name), so one file serves all your projects. All git worktrees of the same repository share the same project namespace in this store.
