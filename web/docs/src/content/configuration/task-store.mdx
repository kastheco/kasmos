...
the task store includes a `signals` table for daemon coordination. signals transition through `pending`, `processing`, `done`, and `failed` states. agents should use mcp `signal_create` to create signals; `kas signal emit` is the operator and cli fallback. see [signals](/cli-reference/signal) for details.
