...
The task store includes a `signals` table for daemon coordination. Signals transition through `pending`, `processing`, `done`, and `failed` states. Use `kas signal emit` to create signals programmatically — this is the primary DB-backed entrypoint. See [signals](/cli-reference/signal) for details.
