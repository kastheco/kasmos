...
kasmos writes this file on first boot (`kas setup` or first interactive launch). If `config.toml` does not exist, a default is created. If only a legacy `config.json` exists, its settings are imported into `config.toml` automatically.
