...
A task is a unit of implementation work tracked through the lifecycle `ready → planning → implementing → reviewing → done`.
...
signals are durable. agents emit signals via `kas signal emit`, which writes a record to the SQLite-backed signals table. the daemon picks them up when it's running — it claims pending signals atomically and drives FSM transitions.
...
yes. the `--program` flag (or `defaultProgram` in config) selects which agent binary to invoke. kasmos has been used with claude code, aider, codex, gemini, opencode, and amp. each agent profile can choose tmux or headless execution independently.
