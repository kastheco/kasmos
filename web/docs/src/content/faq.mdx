...
`kas` is the CLI command name — it's what you type in your terminal. `kasmos` is the binary artifact name produced by the build. After installation, `just install` symlinks both `kas` and `kms` to the same binary. In source code and docs you'll see both names; they refer to the same program.
...
a task is a unit of implementation work tracked through the lifecycle `ready → planning → planned → implementing → reviewing → done`. when `auto_readiness_review = true`, a master agent runs a holistic readiness check between reviewer approval and `done`; this appears as the `readiness_reviewing` execution phase within the `reviewing` status.
...
| `reviewing` | a reviewer agent is checking the implementation (may include a `readiness_reviewing` sub-phase if `auto_readiness_review = true`) |
