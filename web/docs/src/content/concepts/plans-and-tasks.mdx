...
The markdown file on disk holds the task **content**. Status, branch, topic, timestamps, and subtask state live in the task store — a SQLite database at `~/.config/kasmos/taskstore.db` (shared across all projects on the machine).
...
For very small tasks (configurable via `blueprint_skip_threshold` in `.kasmos/config.toml`), kasmos skips wave orchestration entirely and hands the whole task to a single coder agent. See [waves](/docs/concepts/waves) for details.
