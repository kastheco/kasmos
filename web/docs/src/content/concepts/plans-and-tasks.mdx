...
For very small tasks (configurable via `blueprint_skip_threshold` in `.kasmos/config.toml`), kasmos skips wave orchestration entirely and hands the whole task to a single coder agent. See [waves](/concepts/waves) for details.
