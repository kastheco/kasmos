...
| `verifying` | a master agent is running the holistic readiness gate before `done` (only entered when `auto_readiness_review = true`); the verify loop is capped by `readiness_max_verify_cycles` — a terminal `verify_failed` is automatically promoted to `verify_approved` with a `loop_cap_exceeded` annotation in the payload |
...
| `verify_approved` | no | master agent approved during verifying (→ `done`); also emitted automatically by the daemon when `readiness_max_verify_cycles` is reached, with a `loop_cap_exceeded` annotation in the payload |
