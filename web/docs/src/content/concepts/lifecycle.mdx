...
The `planner_finished` event writes `ExecutionState{Phase: "planned"}` via `TransitionExecutionState` in `config/taskfsm/fsm.go`. All other events leave the phase empty; the orchestration engine writes it directly as agent sessions advance.
...
Use `taskstate.IsDraftReady(entry)` and `taskstate.IsPlannedReady(entry)` to distinguish them programmatically.
