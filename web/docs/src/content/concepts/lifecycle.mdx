...
| `readiness_reviewing` | master agent is running the holistic readiness gate after reviewer approval (enabled by default; disable with `auto_readiness_review = false`) |
...
| `review_approved` | no | reviewer approved the implementation; intercepted when `auto_readiness_review = true` |
| `review_changes_requested` | no | reviewer requested changes |
| `readiness_approved` | no | master agent approved during readiness review (→ `done`) |
| `readiness_changes_requested` | no | master agent requested changes during readiness review (→ `implementing`) |
...
> **Readiness review signals:** `readiness_approved` and `readiness_changes_requested` are emitted by the master agent during the optional readiness gate phase. They are only accepted while the task's execution phase is `readiness_reviewing`. Aliases: `readiness-approved`, `readiness-changes`, `readiness-changes-requested`, `master_approved`.
...
reviewing
  ├─ review_approved     → done  [or → readiness_reviewing phase when auto_readiness_review=true]
  ├─ review_changes_requested → implementing
  └─ cancel              → cancelled

  (readiness_reviewing sub-phase — reviewing status, master agent active)
  ├─ readiness_approved          → done
  └─ readiness_changes_requested → implementing
