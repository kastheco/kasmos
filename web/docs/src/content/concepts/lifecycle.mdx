...
| `verifying` | a master agent is running the holistic readiness gate before `done` (only entered when `auto_readiness_review = true`); the verify loop is capped by `readiness_max_verify_cycles` — a terminal `verify_failed` is automatically promoted to `verify_approved` and the task transitions `verifying → done` |
...
| `verify_approved` | no | master agent approved during verifying (→ `done`); the daemon also synthesizes this transition when `readiness_max_verify_cycles` is reached — no distinct payload annotation is added, but the processor flags the internal action as force-promoted so callers (e.g. the TUI) can surface a warning |
| `verify_failed` | no | master agent requested changes during verifying (→ `implementing`); when the verify-round count reaches `readiness_max_verify_cycles` the daemon intercepts this and force-promotes to `verify_approved` instead |
