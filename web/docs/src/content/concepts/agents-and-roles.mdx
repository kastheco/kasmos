...
| `master` | `readiness_review` | holistic readiness reviewer; runs after reviewer approval (enabled by default; disable with `auto_readiness_review = false`). Config alias `master_review` is accepted for backward compatibility. |
...
fixer          = "fixer"
readiness_review = "master"   # canonical key; master_review is accepted as a backward-compat alias
