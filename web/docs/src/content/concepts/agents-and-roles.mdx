...
| `master` | `verifying` | holistic readiness reviewer; spawned when the task enters the `verifying` status after reviewer approval (enabled by default; disable with `auto_readiness_review = false`). Config alias `master_review` is accepted for backward compatibility. |
