...
| `master` | `readiness_review` | holistic readiness reviewer; runs after reviewer approval when `auto_readiness_review = true` (opt-in gate). Config alias `master_review` is accepted for backward compatibility. |
