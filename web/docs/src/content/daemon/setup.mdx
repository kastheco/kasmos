...
The daemon reads `~/.config/kasmos/daemon.toml` on startup. Create it manually or let the daemon create an empty one on first run. See the [CLI reference](/cli-reference/daemon#configuration-file) for the full field list.
