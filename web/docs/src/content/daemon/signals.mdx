...
| `review_approved` | reviewer | FSM transition → `done` (or → `readiness_reviewing` phase when `auto_readiness_review = true`) |
| `review_changes_requested` | reviewer | FSM transition → `implementing`, spawn fixer |
| `planner_finished` | planner | FSM transition → `ready` |
| `readiness_approved` | master agent | FSM transition → `done` (only valid during `readiness_reviewing` phase) |
| `readiness_changes_requested` | master agent | FSM transition → `implementing`, spawn fixer (only valid during `readiness_reviewing` phase) |
...
> **Readiness signals:** `readiness_approved` and `readiness_changes_requested` are only accepted while the task's execution phase is `readiness_reviewing` (master agent active). Aliases accepted: `readiness-approved`, `readiness-changes`, `readiness-changes-requested`, `master_approved`. These signals are emitted by the master agent as its final verdict on the full implementation.
