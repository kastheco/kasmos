...
The **primary signal path is the DB-backed gateway** — signals are rows in the `signals` SQLite table inside each repo's `taskstore.db`. This path is atomic, auditable, and safely concurrent. A secondary **filesystem path** exists for compatibility and is automatically bridged into the DB on every daemon tick.
...
### writing a filesystem signal directly

Legacy agents or low-level tooling can write to the filesystem path by calling `taskfsm.AtomicWrite` in `config/taskfsm/signalfs.go`. The write is atomic: the payload is staged under `staging/` and renamed into the signals root in a single syscall. The daemon bridge picks it up on the next tick and promotes it to a DB-backed row.

> **Note:** `kas signal emit` routes to the DB gateway, not the filesystem. Use it from the command line to write DB-backed signals; the filesystem path is for legacy agents that cannot call the gateway directly.
