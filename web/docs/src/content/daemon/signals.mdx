...
| `verify_approved` | master agent or daemon (loop-cap promotion) | FSM transition `verifying → done`; when emitted by the daemon as a loop-cap promotion the payload contains a `loop_cap_exceeded` annotation with the original master findings |
