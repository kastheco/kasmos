...
This is a narrower FSM transition: it is only valid when the task is already in the `done` state (see `config/taskfsm/fsm.go`). It moves `done → reviewing` and spawns a reviewer — useful to re-review a branch that was marked done but not yet merged. In the TUI this action appears in the context menu as **request review** only when the task is in `done`.
