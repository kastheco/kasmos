...
The browser preview panel is read-only. You can watch agent output live but cannot send input from the web UI; use the TUI or `kas instance send` for that.
...
- [agents and roles](/concepts/agents-and-roles) — execution modes, headless vs tmux
- [worktrees](/concepts/worktrees) — how isolated branches are managed
