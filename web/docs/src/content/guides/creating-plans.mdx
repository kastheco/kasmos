...
3. stores the content back via `kas task update-content`
4. emits a `planner_finished` signal to notify the orchestration loop
...
See [lifecycle](/concepts/lifecycle) for the full FSM and all available transitions.
...
See [topics](/concepts/topics) for how topics enforce scheduling constraints.
