...
TUI / CLI action
  ...
  (session/ + session/tmux/ or session/headless/)
      ...
      ▼
  agent emits signal
  (kas signal emit ... — DB gateway, preferred)
...
- **Signals** use a DB-backed gateway as the primary path — `kas signal emit` writes rows to the global `~/.config/kasmos/taskstore.db`. The daemon bridges repo-local `.kasmos/signals/` sentinel files into the DB on each tick; this filesystem path is compatibility plumbing, not the recommended workflow for new agents.
...
see the individual docs pages for deeper dives: [lifecycle](/concepts/lifecycle) covers the task FSM, [waves](/concepts/waves) covers wave orchestration, [signals](/daemon/signals) covers the signal gateway, and [daemon architecture](/daemon/architecture) covers the daemon internals.
