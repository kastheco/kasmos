...
| `review_approved` | `reviewing` → `done` |
...
kas task recover my-feature --action readiness-approved
kas task recover my-feature --action readiness-changes --feedback "spec section 3 is not addressed"
```

| flag | description |
|------|-------------|
| `--action` | the recovery action to perform (see table below) |
| `--feedback` | feedback text; required for `review-changes`, `readiness-changes`, and `advance-review-cycle` |
...
| `readiness-approved` | `readiness_approved` | only valid during `readiness_reviewing` phase |
| `readiness-changes` / `readiness-changes-requested` | `readiness_changes_requested` | only valid during `readiness_reviewing` phase; requires `--feedback` |

`readiness-approved` and `readiness-changes` are the manual recovery path for the master readiness gate. Use them when the master agent session ended before it could emit a signal. The `kas status` command shows these hints automatically when a task is stuck in the `readiness_reviewing` execution phase.
