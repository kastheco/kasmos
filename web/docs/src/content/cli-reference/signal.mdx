...
| `review_approved` | optional JSON or text | reviewer approved the implementation; may be intercepted when `auto_readiness_review = true` |
| `review_changes_requested` | optional JSON or text | reviewer requested changes |
| `readiness_approved` | optional JSON or text | master agent approved during readiness review (→ `done`); only valid during `readiness_reviewing` phase. Aliases: `readiness-approved`, `master_approved` |
| `readiness_changes_requested` | optional JSON or text | master agent requested changes during readiness review (→ `implementing`); only valid during `readiness_reviewing` phase. Aliases: `readiness-changes`, `readiness-changes-requested` |
...
- **FSM signals** (`planner_finished`, `implement_finished`, `review_approved`, `review_changes_requested`, `readiness_approved`, `readiness_changes_requested`): payload is optional. If provided and valid JSON, it is stored verbatim. Plain text is wrapped as `{"body": "..."}`.
