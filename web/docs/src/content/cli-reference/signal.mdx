...
| `verify_approved` | optional JSON or text | master agent approved during verifying (→ `done`); only valid while task is in `verifying` status |
| `verify_failed` | optional JSON or text | master agent requested changes during verifying (→ `implementing`); only valid while task is in `verifying` status |
