...
| `verify_approved` | optional JSON or text | master agent approved during verifying (→ `done`); also emitted by the daemon as a loop-cap promotion when `readiness_max_verify_cycles` is reached — the payload then contains a `loop_cap_exceeded` annotation with the original findings; only valid while task is in `verifying` status |
