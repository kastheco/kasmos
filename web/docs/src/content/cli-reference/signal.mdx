...
| `verify_approved` | optional JSON or text | master agent approved during verifying (→ `done`); the daemon also synthesizes this transition internally when `readiness_max_verify_cycles` is reached (no extra payload annotation is written — the processor flags the emitted action as force-promoted so callers can surface a warning); only valid while task is in `verifying` status |
| `verify_failed` | optional JSON or text | master agent requested changes during verifying (→ `implementing`); when the verify-round count reaches `readiness_max_verify_cycles` the daemon intercepts this and force-promotes to `verify_approved` instead; only valid while task is in `verifying` status |
