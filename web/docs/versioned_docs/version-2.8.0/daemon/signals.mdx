...
signals do not carry permission policy. when a signal produces a spawn action, the daemon resolves the target role's profile at dispatch time and puts the final `SkipPermissions` value into `loop.SpawnOpts`. this keeps old signal payloads compatible while allowing repo config to choose prompt or bypass behavior per role.
...
| `verify_approved` | master agent | FSM transition `verifying → done`. **No loop-cap signal is ever written to the gateway.** |
...
> **parallel architect baseline:** the `architect baseline` session is default-on, is not a lifecycle phase, and emits no signal. `plan_start` clears the advisory cache and spawns the planner plus the baseline session by default; normal progress still flows through `planner_finished` and then `elaborator_finished`. Set `[orchestration].parallel_planner_architect = false` to disable.
