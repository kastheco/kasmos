...
Runtime artifacts that are derived from agent work live under the repo-local `.kasmos/cache/` directory. Architect passes write their wave metadata to `.kasmos/cache/<task>-architect.json`; newer metadata may include a schema-versioned `decision_audit` object with the planner summary, architect baseline summary, final decision, and differences. The parallel `.kasmos/cache/<task>-architect-baseline.json` file is advisory input for that audit, not task-store state.
