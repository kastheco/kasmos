...
```
kas monitor [--socket <path>] [--repo <path>] [--plan <slug>] [--kind <kind>] [--since <rfc3339-or-rfc3339nano>] [--json]
```
...
# filter wave failures since a timestamp
kas monitor --kind wave_failed --since 2026-04-24T12:00:00Z
...
| `--kind` | — | filter events to one or more event kinds; repeat for multiple kinds |
| `--since` | — | filter events after an RFC3339 timestamp; fractional RFC3339Nano timestamps copied from daemon JSON are accepted |
