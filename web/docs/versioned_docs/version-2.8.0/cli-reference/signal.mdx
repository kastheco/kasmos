...
Signals describe lifecycle progress only. Permission behavior for any agent spawned as a result of a signal is resolved later by the daemon or tui from `[agents.<role>].permission_default`; the resolved boolean is passed to the session runtime as `SkipPermissions`.
...
> **Parallel architect baseline:** the architect baseline is default-on and does not add a signal type. The advisory `architect baseline` runtime session writes only `.kasmos/cache/<plan-file>-architect-baseline.json`; the planner still emits `planner_finished`, and the final architect pass still completes with `elaborator_finished`. Set `[orchestration].parallel_planner_architect = false` to disable.
