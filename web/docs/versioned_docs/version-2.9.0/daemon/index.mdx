...
optional daemon goroutines run beside that core loop. the pr monitor polls github review state when `[pr_monitor].enabled = true`; the linear trigger monitor polls configured Linear labels and `/kasmos ...` comments only for repos whose `.kasmos/config.toml` enables `[linear.triggers]`.
...
when the daemon spawns an agent, it resolves that role's `[agents.<role>].permission_default` from the repo-local `.kasmos/config.toml` before building `loop.SpawnOpts`. unset or `"inherit"` keeps the daemon's historical unattended default (`SkipPermissions=true`); `"prompt"` forces permission prompts, and `"bypass"` forces the harness bypass behavior. it also forwards the repo's resolved `[resources]` profile through `loop.SpawnOpts.ResourceControls` so planner, architect, coder, reviewer, fixer, master, and solo agents inherit the same process wrapper and build-env policy. the resolved permission value and non-normal resource profile are surfaced through daemon instance status so tui, live-preview, and web-admin clients do not re-derive them.
...
**Linear trigger automation** — the optional linear trigger monitor lets allowlisted Linear users drive guarded `create`, `plan`, and `start` requests from configured labels or strict `/kasmos ...` comments. see [linear triggers](../guides/linear-triggers.mdx) for the repo-local safety model.
...
- [linear triggers](../guides/linear-triggers.mdx) — guarded Linear-to-kasmos task starts
