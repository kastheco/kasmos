...
**solo pill.** instance rows in the web admin ui that were spawned as ad-hoc solo agents (via `start_solo` task variant or the tui `N`/`s`/`S` spawn paths) display a `solo` pill badge next to the instance title. the pill is purely informational — it signals that the row is a standalone agent rather than part of a wave execution plan.
...
**web admin ui — daemon-managed sdk agents.** rows where the daemon owns the session use structured preview and a live composer in the browser preview panel. the daemon exposes dedicated capture and send routes, so the preview refreshes in near-real-time and you can send input directly from the browser without a tmux pane. these rows display the full three-dot action menu.
...
**solo pill.** rows spawned as ad-hoc solo agents display a `solo` pill badge next to the instance title. the pill appears for both tmux and daemon-managed sdk solo rows.

**preview routing by execution mode:**

- **daemon-managed sdk rows** (including solo sdk agents spawned through the daemon): the browser preview panel uses the daemon's structured capture route and renders a live `agent preview` view with a usable composer. the preview updates continuously while the agent runs.
- **legacy standalone sdk rows** (not daemon-managed): the preview panel and composer are unavailable — a placeholder message explains that pane output is not available for these rows. this is a known limitation; no action menu items are shown.
- **tmux rows**: the preview panel captures output from the tmux pane. if the pane exits or the tmux session is killed, the panel transitions to a `session ended` label and the composer is disabled. this state clears when the session is restarted or resumed.
