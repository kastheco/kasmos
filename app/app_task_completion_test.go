...
func TestMetadataResultMsg_DaemonManagedRepoAddsMissingDaemonWaveTaskExactlyOnce(t *testing.T) {
	t.Run("adds and deduplicates restored daemon wave task", func(t *testing.T) {
		dir := t.TempDir()
		...
		assert.Equal(t, 2, instances[0].ReviewCycle)

		duplicateModel, _ := updated.Update(msg)
		duplicate := duplicateModel.(*home)
		require.Len(t, duplicate.nav.GetInstances(), 1)
		assert.Equal(t, 1, restoreCalls)
	})

	t.Run("ignores blank daemon titles before restore", func(t *testing.T) {
		dir := t.TempDir()
		...
		msg, ok := cmd().(metadataResultMsg)
		require.True(t, ok)
		assert.Empty(t, msg.DaemonInstances)
