...
// Instead, pendingApprovals is set and a confirmation overlay appears.
func TestReviewApprovedSignal_SetsPendingApproval(t *testing.T) {
	...
	signal := planfsm.Signal{Event: planfsm.ReviewApproved, PlanFile: planFile}
	msg := metadataResultMsg{PlanState: ps, Signals: []planfsm.Signal{signal}}
...
func TestReviewerDeath_ShowsApprovalPopup(t *testing.T) {
	...
	require.NoError(t, err)
	reviewerInst.IsReviewer = true
	reviewerInst.MarkStartedForTest()
	reviewerInst.SetStatus(session.Running)
	...
	assert.True(t, h.pendingApprovals[planFile],
		"reviewer death popup path must set pendingApprovals for follow-up merge/pr flow")

	assert.Equal(t, stateConfirm, h.state,
		"confirmation overlay must be shown for reviewer death")
}

// TestFocusExit_ShowsMergeOrPRPopupForPendingApproval verifies that exiting
// focus mode from a reviewer with pending approval opens popup 2 (merge/pr).
func TestFocusExit_ShowsMergeOrPRPopupForPendingApproval(t *testing.T) {
	const planFile = "2026-02-23-feature.md"
	...
	list := ui.NewList(&sp, false)
	_ = list.AddInstance(reviewerInst)
	list.SelectInstance(reviewerInst)
	...
	model, _ := h.Update(tea.KeyMsg{Type: tea.KeyCtrlAt})
	updated, ok := model.(*home)
	require.True(t, ok)

	assert.Equal(t, stateConfirm, updated.state,
		"exiting focus mode with pending approval must show merge/pr popup")
	require.NotNil(t, updated.confirmationOverlay)
	assert.Equal(t, "m", updated.confirmationOverlay.ConfirmKey)
	assert.Equal(t, "p", updated.confirmationOverlay.CancelKey)
	assert.NotNil(t, updated.pendingApprovalPRAction,
		"create-pr action should be wired on popup cancel key")
}

// TestReviewerDeath_ApproveThenExitFocus_ShowsMergeOrPRPopup verifies the full
// path-b flow: reviewer death popup -> y -> focus mode -> exit focus -> popup 2.
func TestReviewerDeath_ApproveThenExitFocus_ShowsMergeOrPRPopup(t *testing.T) {
	const planFile = "2026-02-23-feature.md"
	...
	h := &home{
		...
		previewTerminalInstance: reviewerInst.Title,
		activeRepoPath:          dir,
		program:                 "claude",
	}

	deadMsg := metadataResultMsg{
		PlanState: ps,
		Results: []instanceMetadata{
			{Title: reviewerInst.Title, TmuxAlive: false, ContentCaptured: true},
		},
	}

	model, _ := h.Update(deadMsg)
	step1 := model.(*home)
	require.Equal(t, stateConfirm, step1.state)
	require.True(t, step1.pendingApprovals[planFile])

	model, cmd := step1.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("y")})
	step2 := model.(*home)
	require.NotNil(t, cmd)

	model, _ = step2.Update(cmd())
	step3 := model.(*home)
	require.Equal(t, stateFocusAgent, step3.state)

	model, _ = step3.Update(tea.KeyMsg{Type: tea.KeyCtrlAt})
	step4 := model.(*home)
	require.Equal(t, stateConfirm, step4.state)
	require.NotNil(t, step4.confirmationOverlay)
