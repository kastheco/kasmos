...
func (h helpTypeGeneral) toContent() string {
	content := lipgloss.JoinVertical(lipgloss.Left,
		...
		keyStyle.Render("shift+space")+descStyle.Render("   - command launcher"),
		keyStyle.Render("S")+descStyle.Render("             - spawn agent"),
		keyStyle.Render("s")+descStyle.Render("             - quick launch"),
		keyStyle.Render("i")+descStyle.Render("             - interactive mode / send prompt"),
		keyStyle.Render("ctrl+space")+descStyle.Render("    - exit fullscreen or interactive mode"),
		keyStyle.Render("ctrl+shift+enter")+descStyle.Render(" - submit + exit interactive mode"),
		keyStyle.Render("ctrl+k")+descStyle.Render("        - kill tmux session (keeps instance)"),
		keyStyle.Render("ctrl+shift+k")+descStyle.Render("  - stop session (branch preserved)"),
		...
		keyStyle.Render("1/2/3")+descStyle.Render("         - send digit to agent pane"),
		descStyle.Render("agent profiles can choose tmux or sdk execution; tmux stays attachable, sdk favors automated wave work."),
		descStyle.Render("sdk sessions are not attachable; use the preview tab, logs, and prompt overlay while they run."),
		"",
		headerStyle.Render("plans:"),
		keyStyle.Render("n")+descStyle.Render("             - new plan"),
		keyStyle.Render("space")+descStyle.Render("         - instance menu or toggle plan/topic/history"),
...
func (h helpTypeInstanceStart) toContent() string {
	content := lipgloss.JoinVertical(lipgloss.Left,
		...
		descStyle.Render("interactive attach is only available for tmux sessions; sdk sessions use preview/log output."),
		"",
		headerStyle.Render("managing:"),
		keyStyle.Render("↵/o")+descStyle.Render("   - attach to session"),
		keyStyle.Render("tab")+descStyle.Render("   - cycle panes"),
		keyStyle.Render("!")+descStyle.Render("     - interactive + shell mode"),
		keyStyle.Render("ctrl+k")+descStyle.Render(" - kill tmux session"),
		keyStyle.Render("ctrl+shift+k")+descStyle.Render(" - stop session (branch preserved)"),
		"",
