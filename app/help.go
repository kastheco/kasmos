...
func (h helpTypeGeneral) toContent() string {
	content := lipgloss.JoinVertical(lipgloss.Left,
		...
		keyStyle.Render("n")+descStyle.Render("         - new session"),
		keyStyle.Render("N")+descStyle.Render("         - new session with a prompt"),
		keyStyle.Render("S")+descStyle.Render("         - new session (skip permissions)"),
		keyStyle.Render("K")+descStyle.Render("         - kill selected session"),
		keyStyle.Render("↑/j, ↓/k")+descStyle.Render("  - navigate sessions"),
		...
		keyStyle.Render("tab")+descStyle.Render("       - cycle panes: sidebar → agent → diff → git → list"),
		keyStyle.Render("shift+tab")+descStyle.Render(" - cycle panes backward"),
		keyStyle.Render("!/@ /#")+descStyle.Render("    - jump to agent/diff/git tab"),
		keyStyle.Render("s")+descStyle.Render("         - jump to sidebar"),
		keyStyle.Render("g")+descStyle.Render("         - open git tab (lazygit, ctrl+space to exit)"),
		keyStyle.Render("↑/k, ↓/j")+descStyle.Render("  - navigate within focused pane"),
		keyStyle.Render("←/h, →/l")+descStyle.Render("  - tree expand/collapse (sidebar)"),
		...
		keyStyle.Render("↵/o")+descStyle.Render("       - context menu or run stage"),
...
func (h helpTypeInstanceStart) toContent() string {
	content := lipgloss.JoinVertical(lipgloss.Left,
		...
		keyStyle.Render("↵/o")+descStyle.Render("   - attach to session"),
		keyStyle.Render("tab")+descStyle.Render("   - cycle panes (!/@ /# to jump to agent/diff/git)"),
		keyStyle.Render("K")+descStyle.Render("     - kill selected session"),
		"",
