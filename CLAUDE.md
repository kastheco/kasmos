...
- Repo-local config still lives under `<repo-root>/.kasmos/` via `config/config.go:GetConfigDir`, but the default task store is the global shared SQLite DB at `~/.config/kasmos/taskstore.db` (fallback `./.kasmos/taskstore.db` only when no home dir is available). `config.toml` is the authoritative live config, and `config.json` only survives as a narrow one-time import/migration path handled by `config/config.go:migrateJSONToTOML`.
