...
// boundary for extension-less store keys; deeper FSM/signal layers preserve
// whatever filename they receive and rely on CLI/MCP ingress or this migration
// for normalization. OR IGNORE skips any row where stripping '.md' would
// collide with an already-existing bare-slug entry, so the migration is safe to
// run on databases that were partially updated.
