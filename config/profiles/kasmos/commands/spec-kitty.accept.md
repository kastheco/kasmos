...
0. **Smoke-import gate** (auto-detected, mandatory for Python features):
   - Scan the feature's changed files for any Python packages (directories containing `__init__.py`).
   - For each discovered package, run:
     ```bash
     python -c "import <package_dotted_path>"
     ```
   - If ANY import fails, **stop immediately** and report the ImportError with the exact broken name and source module. Do NOT proceed to the acceptance CLI.
   - This catches cross-WP integration bugs (name drift between `__init__.py` re-exports and actual implementations) that per-WP reviews may miss.
