...
- Keep `/spec-kitty.tasks` on the controller agent (heavy tier) for high-quality decomposition.
- Do not delegate task decomposition and dependency design to light-tier agents.
- Profile default: `controller` -> `anthropic/claude-opus-4-6` with `reasoningEffort: high`.
