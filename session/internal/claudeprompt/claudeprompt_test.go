...
func TestFind(t *testing.T) {
	tests := []struct {
		...
		{
			name: "trust screen with plan content is not a prompt",
			content: strings.Join([]string{
				`"preferred_model": "openai/gpt-5.3-codex-spark-5"`,
				`47  [coverage:run]`,
				`48  include = app/*`,
				``,
		...
		{
			name: "distant file content not used as description",
			content: strings.Join([]string{
				`47  [coverage:run]`,
				`48  include = app/*`,
				`49  omit =`,
				...
				`Bash command`,
				``,
				`  rg "^def create" /home/user/project/app/security.py --no-filename -n`,
				`  Run shell command`,
				``,
				`This command requires approval`,
				``,
				`Do you want to proceed?`,
				`) 1. Yes`,
				`  2. Yes, and don't ask again for: rg ^def create:*`,
				`  3. No`,
				``,
				`Esc to cancel · Tab to amend · ctrl+e to explain`,
			}, "\n"),
