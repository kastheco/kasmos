...
>
> for current lifecycle/operator guidance, see [`docs/lifecycle-operator-guide.md`](./lifecycle-operator-guide.md).
>
...
- operator-facing lifecycle terms now come from the layered `status` + `execution_phase` model; `elaborator_finished` remains a compatibility wire name only.
