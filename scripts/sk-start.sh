...
Notes:
  ...
  - Auto-commit only stages files under code directories: crates/, src/, tests/, benches/,
    docs/, scripts/, Cargo.toml, Cargo.lock, build.rs, .gitignore.
  - All tool/ephemeral state (.kittify/, .kas/, .opencode/, IMPLEMENT.md, etc.) is ignored.
EOF
}
...
ensure_worktree_gitignore() {
  local worktree="$1"
  local gi="$worktree/.gitignore"

  # Ephemeral/tool paths that must never be committed or seen as untracked.
  # Only non-comment entries are checked for presence — the marker comment is
  # written once and then used as a fast-path sentinel on subsequent calls.
  local -a IGNORE_ENTRIES=(
    ".kas/"
    ".opencode/"
    ".kittify/"
    ".claude/"
  ...
  local MARKER="# Auto-generated by kasmos swarm — do not commit tool state"

  # Fast path: marker present means we already ran to completion once.
  if [[ -f "$gi" ]] && grep -qF "Auto-generated by kasmos swarm" "$gi" 2>/dev/null; then
    return 0
  fi

  # Slow path: check each entry individually and only append missing ones.
  # This avoids duplicating entries that spec-kitty CLI (or the user) already added.
  local -a missing=()
  ...
  if [[ "${#missing[@]}" -eq 0 ]]; then
    # All entries present — just stamp the marker so we take the fast path next time.
    printf '\n%s\n' "$MARKER" >> "$gi"
  else
    # Append a block with marker + missing entries.
    {
      printf '\n%s\n' "$MARKER"
      printf '%s\n' "${missing[@]}"
    } >> "$gi"
  ...
  # Stage .gitignore immediately so spec-kitty preflight never sees it as dirty.
...
auto_commit_wp_changes() {
  ...
  # Ensure ephemeral files are gitignored so spec-kitty preflight passes.
  ensure_worktree_gitignore "$worktree"
  ...
  # Allowlist: only these top-level directories/files contain deliverable code.
  # Everything else (tool state, ephemeral prompts, agent artifacts) is skipped.
  # .gitignore is included because ensure_worktree_gitignore() may modify it.
  local -a CODE_DIRS=(
    "crates/"
    "src/"
    "tests/"
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    log "[dry-run] git -C \"$worktree\" add -u -- ${CODE_DIRS[*]}"
    log "[dry-run] git -C \"$worktree\" add <new files under CODE_DIRS>"
    log "[dry-run] git -C \"$worktree\" commit -m \"$commit_msg\""
    return 0
  fi

  # Stage tracked modifications ONLY under code directories.
  local -a existing_dirs=()
  for d in "${CODE_DIRS[@]}"; do
    if [[ -e "$worktree/$d" ]]; then
      existing_dirs+=("$d")
    fi
  done

  if [[ "${#existing_dirs[@]}" -gt 0 ]]; then
    git -C "$worktree" add -u -- "${existing_dirs[@]}"
  fi

  # Stage new (untracked) files ONLY under code directories.
  ...
  if git -C "$worktree" diff --cached --quiet; then
    log "- $wp: no code changes to commit"
    return 0
  fi

  log "- $wp: staging for commit:"
...
start_review() {
  ...
  # Always ensure gitignore so spec-kitty preflight doesn't see tool state as dirty.
  [[ -d "$worktree" ]] && ensure_worktree_gitignore "$worktree"
