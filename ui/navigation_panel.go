...
func navInstanceTitle(inst *session.Instance) string {
	switch {
	case inst.AgentType == "architect-baseline" && inst.TaskFile != "":
	...
	case inst.AgentType == session.AgentTypeCoder && inst.TaskFile != "" && inst.WaveNumber == 0:
		return "implementing"
	case inst.AgentType == session.AgentTypeFixer && inst.TaskFile != "" && inst.WaveNumber == 0:
		if inst.ReviewCycle > 0 {
			return fmt.Sprintf("applying fixes #%d", inst.ReviewCycle)
		}
		return "applying fixes"
	case inst.AgentType == session.AgentTypeMaster && inst.TaskFile != "":
		return "readiness review"
