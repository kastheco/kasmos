...
func TestPromptTemplates_UseMCPFirstSignals(t *testing.T) {
	tests := []struct {
		...
		{
			name: "master skill uses readiness-specific signal types via MCP and CLI fallback",
			path: "templates/skills/kasmos-master/SKILL.md",
			contains: []string{
				`signal_type: "readiness-approved"`,
				`signal_type: "readiness-changes-requested"`,
				"kas signal emit readiness_approved",
				"kas signal emit readiness_changes_requested",
				"readiness_reviewing",
				"readiness gate",
			},
			notContains: []string{
		...
		{
			name: "lifecycle skill documents readiness_reviewing phase and signal types",
