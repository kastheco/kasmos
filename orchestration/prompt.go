...
func BuildTaskPrompt(planFile string, plan *taskparser.Plan, task taskparser.Task, waveNumber, totalWaves, peerCount int, meta *TaskMeta) string {
	...
	sb.WriteString(fmt.Sprintf("- When done: signal completion with MCP `signal_create` (signal_type: \"implement-task-finished\", plan_file: \"%s\", payload: \"{\\\"wave_number\\\":%d,\\\"task_number\\\":%d}\"). Then stop.\n\n",
