...
func TestProcessor_LifecycleSignalMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			signal:        taskfsm.Signal{TaskFile: "plan.md", Event: taskfsm.VerifyApproved, Body: "ready", Origin: "master", ReviewedSHA: verificationHead},
			autoReadiness: true,
			wantStatus:    taskstore.StatusDone,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p := NewProcessor(ProcessorConfig{Store: store, Project: project, AutoReviewFix: tt.autoFix, AutoReadinessReview: tt.autoReadiness, HeadSHA: func(string) (string, error) { return verificationHead, nil }})
