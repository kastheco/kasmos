...
func TestBuildMasterReviewPrompt(t *testing.T) {
	...
	assert.Contains(t, prompt, "actual base branch")
	assert.Contains(t, prompt, `git merge-base HEAD "$BASE_BRANCH"`)
	assert.NotContains(t, prompt, "git merge-base HEAD main")
	...
	assert.Contains(t, prompt, "git restore --staged --worktree -- <paths>")
	assert.NotContains(t, prompt, "git restore --staged --worktree .")
