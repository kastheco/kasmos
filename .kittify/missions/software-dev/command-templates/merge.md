---
description: Merge a completed feature into the master branch and clean up worktree
---

# /spec-kitty.merge - Merge Feature to Master

**Version**: 0.11.0+
**Purpose**: Merge ALL completed work packages for a feature into master branch.
...
- Branch: Should show your feature branch name like `###-feature-name-WP01` (NOT `master` or `release/*`)

**If you see:**
- Branch showing `master` or `release/`
...
**Exception (master branch):**
If you are on `master` and need to merge a workspace-per-WP feature, run:
```bash
...
- You're not attempting to run from `master` or any release branch
...
3. ✅ Working directory must be clean (no uncommitted changes in master)
...
6. **Switches** to the target branch (default: `master`)
...
# Run merge from master for a workspace-per-WP feature
...
# Merge into develop instead of master
...
| `--target` | Target branch to merge into | `master` |
| `--dry-run` | Show what would be done without executing | off |
| `--feature` | Feature slug when merging from master branch | none |
...
```
my-project/                              # Main repo (master branch)
├── .worktrees/
...
- Merges each WP branch into master in sequence
...
### The Rules
1. **Master branch** stays in the primary repo root
...
### "Could not fast-forward master"
Your master branch is behind origin:
```bash
git checkout master
git pull
git checkout <feature-branch>
...
2. **Fast-forward only pull** - Won't proceed if master has diverged
...
After a successful merge, you're back on the master branch with:
...
The `/spec-kitty.merge` command **integrates** your feature into master.
