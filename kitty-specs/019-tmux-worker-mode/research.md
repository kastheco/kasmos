...
|-----------|---------|-------|
| Create pane | `tmux split-window -h -t <target> -P -F '#{pane_id}' <cmd>` | `-P` prints pane info, `-F` formats output |
| Move pane to parking | `tmux break-pane -d -s <pane> -t <window>` | `-d` keeps source window |
| Bring pane from parking | `tmux join-pane -s <src> -t <dst> -h -l 50%` | `-h` horizontal split, `-l` size |
| Focus pane | `tmux select-pane -t <pane>` | Moves keyboard focus |
| List panes | `tmux list-panes -t <target> -F '#{pane_id} #{pane_pid} #{pane_dead} #{pane_dead_status}'` | Format strings for structured parsing |
...
| Set env var on pane | `tmux set-environment -t <pane> KASMOS_WORKER_ID <value>` | For tagging/rediscovery |
| Read env var | `tmux show-environment -t <pane> KASMOS_WORKER_ID` | For pane identification |
| Create window | `tmux new-window -d -n kasmos-parking` | `-d` don't switch to it |
| Get current pane | `tmux display-message -p '#{pane_id}'` | Kasmos's own pane ID |

## 2. Pane Tagging for Rediscovery

### Decision: tmux environment variables per pane

**Rationale**: tmux supports per-session environment variables via `set-environment`.
While tmux 3.0+ added pane options (`set-option -p`), environment variables work
across all tmux 2.x+ versions and are simpler to query.

**Tagging scheme**:
- `KASMOS_SESSION=<session-id>` - Matches the kasmos session ID (e.g., `ks-1739808000-a3f8`)
- `KASMOS_WORKER=<worker-id>` - Matches the worker ID (e.g., `w-001`)

These are set when the pane is created and queried during reattach scan.
...
**Polling behavior**:
- On each tick (tmux mode only), run `tmux list-panes -t <parking-window> -F ...`
  and also check the active pane.
...
### Decision: Hidden parking window with join-pane/break-pane

**Rationale**: A dedicated hidden tmux window named `kasmos-parking` holds all
non-visible worker panes. Showing a worker: `join-pane` from parking into the main
kasmos window. Hiding: `break-pane` back to parking. The visible layout always has
exactly two panes: kasmos (left) + active worker (right).

**Pane swap sequence** (switching from worker A to worker B):
1. `tmux break-pane -d -s <pane-A>` (A goes to parking, kasmos window now has only kasmos pane)
2. `tmux join-pane -s kasmos-parking.<pane-B> -t <kasmos-window> -h -l 50%` (B joins kasmos window)
...
- Multiple windows: Each worker in its own tmux window. User must use tmux window
  switching instead of kasmos table navigation. Breaks the "kasmos dashboard stays
  visible" requirement.
