...
3. **Given** the operator has no orchestration run loaded, **When** they open the TUI, **Then** a meaningful empty state is shown with guidance on how to start a run.
...
- What happens if the orchestration run terminates unexpectedly while the TUI is active? The TUI must show a clear error state rather than hanging.
...
- **FR-005**: System MUST provide contextual action buttons for each WP based on its current state (restart, pause, resume, retry, force-advance, advance).
- **FR-006**: System MUST support both keyboard navigation (vim-style hjkl, tab switching, hotkeys) and mouse input (click, scroll).
...
- **Notification**: An attention item surfaced in the persistent notification bar. Has a type (review, failure, input-needed), a target WP identifier, a creation timestamp, and an optional message (populated for input-needed notifications with the agent's question/context).
...
- **SC-003**: State changes from any source (TUI action, FIFO command, automatic detection) are reflected in the display within 1 second.
